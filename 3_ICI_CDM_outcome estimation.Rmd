...
```{r}
# National death registry linkage 
...
# [27] "K26" 
# Cause-of-death categories: C neoplasm, I circulatory (cerebrovascular), J/A-B infection, U COVID-19, R ill-defined, K duodenal ulcer cancer 
# Exclude S00-T98 (injury and external causes; remove 3 records)
...
# Remove matched pairs censored before the index date
...
death_incidence <- patient_mono_time_ps_demo %>%
  group_by(TARGET) %>%
  summarise(
    total = n(),  # total number of individuals
    deaths = sum(DEATH, na.rm = TRUE),  # number of death events
    incidence_per_1000 = (deaths / total) * 1000  # incidence per 1,000 persons 
  )
...
ggsurv <- ggsurvplot(
  ...
  break.time.by = 100,
  risk.table.breaks = seq(0, 730, by = 100),  # risk table every 100 days
  xlim = c(0, 730)  # limit x-axis to 730 days
)
...
pdf("KM_death_psm_combined.pdf", width = 13, height = 12)
print(ggsurv)
dev.off()



# Inspect censoring distribution
...
censoring_plot <- ggplot(censoring_data, aes(x = death_survival_days, color = Population)) +
  ...
  theme(
    ...
    panel.grid.major = element_line(color = "grey90"),  # lighten major grid lines
    panel.grid.minor = element_line(color = "grey95")  # lighten minor grid lines
  ) +
...
MFS_incidence <- patient_mono_time_ps_demo %>%
  ...
  summarise(
    total = n(),  # total number of individuals
    MFS = sum(MFS, na.rm = TRUE),  # number of progression events
    incidence_per_1000 = (MFS / total) * 1000  # incidence per 1,000 persons
  )
...
censored_ids <- unique(ICI_dc_censor$PERSON_ID)

ICI_dc <- ICI_dc %>%
  mutate(regimen_change = if_else(PERSON_ID %in% censored_ids, NA, regimen_change))
...
change_incidence <- patient_mono_time_ps_demo %>%
  ...
  summarise(
    total = n(),  # total number of individuals
    change = sum(ICI_dc_chemo, na.rm = TRUE),  # number of regimen change events
    incidence_per_1000 = (change / total) * 1000  # incidence per 1,000 persons
  )
...
pdf("KM_ttnt_psm_combined.pdf", width = 13, height = 12)
print(ggsurv)
dev.off()
...
hospital_incidence <- patient_mono_time_ps_demo %>%
  ...
  summarise(
    ...
    incidence_per_1000 = (Hospital / total) * 1000  
  )
...
pdf("KM_hospital_psm_combined.pdf", width = 13, height = 12)
print(ggsurv)
dev.off()
...
Adjust all time-dependent variables; keep baseline events active from the index date
...
#Sepsis
sepsis <- read.csv("sepsis_events.csv")[,-1]
...
# irAE
irAE <- read.csv("irae_events.csv")[,-1]
...
time_cox_model_death <- coxph(Surv(start, stop, Death_event) ~ sepsis_event + irAE_event + AUTOIMMUNE + OPIOID + TARGET, cluster(pair_id), data = death_cox) 
time_cox_model_death <- coxph(Surv(start, stop, Death_event) ~ sepsis_event + irAE_event + AUTOIMMUNE + OPIOID + TARGET + strata(pair_id), data = death_cox)  
time_cox_model_death <- coxph(Surv(start, stop, Death_event) ~ sepsis_event + irAE_event + AUTOIMMUNE + OPIOID + TARGET, data = death_cox)  
...
time_cox_model_death <- coxph(Surv(start, stop, Death_event) ~ AUTOIMMUNE + TARGET, cluster(pair_id), data = death_cox)
summary(time_cox_model_death)


time_cox_model_death <- coxph(Surv(start, stop, Death_event) ~ sepsis_event + AUTOIMMUNE + TARGET, cluster(pair_id), data = death_cox)  
summary(time_cox_model_death)
...
time_cox_model_meta <- coxph(Surv(start, stop, Meta_event) ~ irAE_event + AUTOIMMUNE + OPIOID + hospital_sever + TARGET + strata(pair_id), data = meta_cox) 
summary(time_cox_model_meta)


time_cox_model_meta <- coxph(Surv(start, stop, Meta_event) ~ irAE_event + AUTOIMMUNE + OPIOID + TARGET + strata(pair_id), data = meta_cox) 
summary(time_cox_model_meta)
