...
patient_mono_time_ps <- patient_mono_time_ps %>%
  group_by(pair_id) %>%
  mutate(
    index = if (any(type == "trt")) {
      first(ad_steroid[type == "trt"], default = NA_real_)}
    else {NA_real_},
    N_days = first(
      as.integer(as.Date(Steroid_index_date[type == "trt"]) - as.Date(COHORT_START_DATE[type == "trt"])),
      default = NA_integer_
    ),
    
    index_fu = case_when(
      type == "trt" & ICI_prescrib_TDPS == index ~ as.character(Steroid_index_date),
      type == "all" & ICI_prescrib_TDPS == index ~ as.character(as.Date(COHORT_START_DATE) + N_days),
      type == "all" & !any(type == "all" & ICI_prescrib_TDPS == index, na.rm = TRUE) ~ "Exclusion",
      TRUE ~ NA_character_
    )
  ) %>%
  ungroup()%>%
