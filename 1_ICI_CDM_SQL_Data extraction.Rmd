...
# Additional primary cancer data if required
# Additional metastasis codes 
```{r}
...
Column names retrieved from the 2023 CDM schemas

3. Retrieve death_date from the death table
```{r}
query3 <- "
SELECT
    cohort.subject_id,
...
4. Retrieve gender_concept_id, year_of_birth, and month_of_birth from the person table
# 8507 indicates male
...
6. Retrieve ICU and inpatient visits
** ICU concepts are not mapped in this data source, so inpatient visits are used instead.
Additional mapping may be required for other sites
...
-- Use the SQL below to review mapped concepts within the CDM		
select * from cdm.concept limit 190
select * from cdm.source_to_concept_map where site_source_id similar to 'L20%'
...
# Updated concomitant medication list (2024-05-20)
query8_1 <-"
...
## Smoking: No records found in this cohort
```{r}
...
## Some participants have data, but missingness was too high, so the variable was excluded (2024-10-04).
...
######### 2024-08-26: Re-run data extraction from this point and verify bevacizumab concepts in ATLAS
...
Prognostic factor: autoimmune disease
Subgroup: indications for steroid use and brain metastasis
...
# Added irAE covariates on 2024-05-31
## Update ATLAS definitions if changes are made (2024-09-27).
...
##### Create R subsets from irAE_total based on concept_id
...
### CSC definition concepts: refer to ATLAS or the imported concept list (2024-09-27).

Prognostic factor: EBRT (concept 45949646; not mapped in this data source).
Subgroup: GKS & WBRT concepts 4277939,44791440,44790307,44791428,40488986,764407,4021989,4199388,4244986,4198764.
