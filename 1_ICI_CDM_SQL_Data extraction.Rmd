...
0. Atlas에서 구축한 cohort 데이터 불러들이고, target/comp ID로 저장

```{r}
library(dplyr)

query0 <- "select *
from public.SNUH5ICIentry_0822"
...
# 항목 선별: 4160340 분화도 / 4219603 tnmstaging / 4299435 metacode / 3031228 summary staging

query_check <- "SELECT *
FROM
    cdm2024.observation o
INNER JOIN
    cdm2024.concept_ancestor ca ON o.observation_concept_id = ca.descendant_concept_id
LEFT JOIN
    (SELECT subject_id FROM public.SNUH5ICIentry_0822) cohort
ON
...
query_lung <- "SELECT *
FROM
    cdm2024.condition_occurrence co
INNER JOIN
    cdm2024.concept_ancestor ca ON co.condition_concept_id = ca.descendant_concept_id
LEFT JOIN
    (SELECT subject_id FROM public.SNUH5ICIentry_0822) cohort
ON
...
# 4110591: 제외한 컨셉 small cell lung cancer
...
1. 코호트 내 subject_id에 스테로이드 exposure 데이터 left join
```{r}
# Steroid exposure
query1 <- "
SELECT *
FROM
    cdm2024.drug_exposure de
INNER JOIN
    cdm2024.concept_ancestor ca ON de.drug_concept_id = ca.descendant_concept_id
LEFT JOIN
    (SELECT subject_id FROM public.SNUH5ICIentry_0822) cohort
ON
...
2023 SNUH CDM schemas에서 불러올 column명

3. death table: death_date 불러오기
```{r}
query3 <- "
SELECT
    cohort.subject_id,
...
FROM
    (SELECT subject_id FROM public.SNUH5ICIentry_0822) cohort
LEFT JOIN
    cdm2024.death d ON cohort.subject_id = d.person_id
"
...
4. person: gender_concept_id/year_of_birth/month_of_birth 불러오기
# 	8507 가 남자
...
SELECT
    ...
    p.month_of_birth
FROM
    public.SNUH5ICIentry_0822 cohort
LEFT JOIN
    cdm2024.person p ON cohort.subject_id = p.person_id
"
...
6. ICU & 일반 입원 불러오기
** 서울대병원에는 ICU 매핑되어 있지 않아서 생략 및 일반입원으로 대체하였음.
다른 병원에서는 추가 필요
...
-- 아래 sql 코드는 cdm 내 매핑되어 있는 컨셉 조회할 때 쓰는 코드		
select * from cdm.concept limit 190
select * from cdm.source_to_concept_map where snuh_id similar to 'L20%' 

select * from cdm.concept
where lower(concept_name) similar to '%neutrophil count%'
and domain_id = 'Measurement'


8. 병용약제 불러오기: 6개월 이내
# 쿼리는 SQL로 돌린 후에 결과 테이블만 불러왔음. 환자당 아무레코드도 없는 경우는 ICI 사용 30일 전에 병용약물 카운트 안된 것.
```{r}
...
SELECT c.subject_id AS person_id,
    ...
    MAX(CASE
        ...
        ELSE 0
        END) AS opioid
FROM cdm2024.drug_exposure de
INNER JOIN public.SNUH5ICIentry_0822 c ON de.person_id = c.subject_id
...
## Smoking: 우리 코호트 내에 없었음
```{r}

# 43055140	72513-5	Vital signs with smoking status and pain scale [VSP]	Survey	0	451,084,834	Observation	LOINC
# 1177395	LG50107-8	Smoking - biochemical markers	LOINC Group	0	56,594	Measurement	LOINC
# 1177408	LG50111-0	Smoking biochemical markers	LOINC Group	0	56,594	Measurement	LOINC

query8_9 <-"
SELECT *
FROM
    cdm2024.observation po
INNER JOIN
    cdm2024.concept_ancestor ca ON po.observation_concept_id = ca.descendant_concept_id
LEFT JOIN
    (SELECT subject_id FROM public.SNUH5ICIentry_0822) cohort
ON
...
SELECT *
FROM
    cdm2024.measurement po
INNER JOIN
    cdm2024.concept_ancestor ca ON po.measurement_concept_id = ca.descendant_concept_id
LEFT JOIN
    (SELECT subject_id FROM public.SNUH5ICIentry_0822) cohort
ON
    po.person_id = cohort.subject_id
...
## 일부 대상자 데이터 있으나 missing 너무 커서 제외 (2024.10.04.)
...
9. 다른 항암제 사용: 기간 제한없이 모두(entry 시점의 다른 항암제 병용은 atlas entry 시 제외, index 이후 다른 항암제는 rwTOT 
+ sensitivity analysis에서 2년이내 다른 항암제 사용 환자만 걸러냄=second-line 이상 monotherapy)

35807304       Taxane                          
35807315        Platinum agent                      
1350504         etoposide   
1367268	51499	irinotecan	Drug	Standard	

1343346	39541	vinorelbine	Drug	Standard			
19008264	11198	vinblastine	Drug	Standard			
1304919	68446	pemetrexed	Drug	Standard		
1314924	12574	gemcitabine	Drug	Standard			
1338512	3639	doxorubicin	Drug	Standard

# ALK inhibitor	35807229
# Tyrosine kinase inhibitor	912198

######### 2024-08-26 여기서부터 다시 데이터 추출하기: atlas 확인해서 bevacizumab 컨셉 추가 필요
...
10. 병존질환 불러오기: 1년 이내
# 쿼리는 SQL로 돌린 후에 결과 테이블만 불러왔음. 환자당 아무레코드도 없는 경우는 ICI 사용 1년 전에 진단코드 없는 것
```{r}
...
FROM cdm2024.condition_occurrence co
INNER JOIN public.SNUH5ICIentry_0822 c ON co.person_id = c.subject_id
...
11~14. 예후인자 불러오기: measurement table (TNM은 따로)
...
(
SELECT * 
FROM cdm2024.measurement m
INNER JOIN public.SNUH5ICIentry_0822 c ON m.person_id = c.subject_id
WHERE
...
select * from cte1 m
INNER JOIN
cdm2024.concept_ancestor ca ON m.measurement_concept_id=ca.descendant_concept_id
"
...
# irAE covariate 추가 _ 2024.05.31
## Atlas 수정 시 (2024.09.27.)
...
##### irAE_total 데이터셋을 기준으로 concept_id로 R에서 subset 만들기
...
select c.concept_name, c.concept_id
  from cdm2024.CONCEPT c
  join cdm2024.CONCEPT_ANCESTOR ca on c.concept_id = ca.descendant_concept_id
...
### CSC 정의 컨셉 포함: 구체적인 목록은 아틀라스 혹은 성모반입컨셉목록240919 확인 _24.09.27.

(예후인자) EBRT	45949646 <= 서울대병원 maaping 0 *****
(하위그룹) GKS & WBRT	4277939,44791440,44790307,44791428,40488986,764407,4021989,4199388,4244986,4198764
...
INNER JOIN
    cdm2024.concept_ancestor ca ON po.procedure_concept_id = ca.descendant_concept_id
LEFT JOIN
    (SELECT subject_id FROM public.SNUH5ICIentry_0822) cohort
  ON
    po.person_id = cohort.subject_id
...
write.csv(addmission, file = "addmission.CSV")
write.csv(autoimmunedz, file = "autoimmunedz.CSV")
write.csv(cancer_agent, file = "cancer_agent.CSV")
write.csv(CBC_lab, file = "CBC_lab.CSV")
write.csv(cytokeratin, file="cytokeratin.CSV")
write.csv(comorbd_disease, file = "comorbd_disease.CSV")
write.csv(concom_drug2, file = "concom_drug2.CSV")
write.csv(death, file = "death.CSV")
write.csv(ER, file = "ER.CSV")
write.csv(ICI, file = "ICI.CSV")
write.csv(patient, file = "patient.CSV")
write.csv(sexbirth, file = "sexbirth.CSV")
write.csv(steroid, file = "steroid.CSV")
# 
# write.csv(RT, file = "RT.CSV")
# write.csv(GKS_WBRT, file = "GKS_WBRT.CSV")
write.csv(brainmeta, file = "brainmeta.CSV")
write.csv(bonemeta, file = "bonemeta.CSV")
write.csv(livermeta, file = "livermeta.CSV")

write.csv(sepsis, file = "sepsis.CSV")
write.csv(dyspnea, file = "dyspnea.CSV")
write.csv(pneumonitis, file = "pneumonitis.CSV")
# write.csv(TNM, file="TNM.CSV")
write.csv(TKI_ALK, file = "TKI_ALK.CSV")


write.csv(pneumonitis, file = "pneumonitis.CSV")
write.csv(Thyrotoxicosis, file = "Thyrotoxicosis.CSV")
write.csv(Cardiomyopathy, file = "Cardiomyopathy.CSV")
write.csv(Dermatotx, file = "Dermatotx.CSV")
# write.csv(pancreatitis, file = "pancreatitis.CSV")
# write.csv(Encephalomyelitis, file = "Encephalomyelitis.CSV")




# write.csv(imsi, file = "imsi.CSV")
# check <- unique(TKI_ALK$PERSON_ID)
